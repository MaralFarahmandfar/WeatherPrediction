...
#بارگذاری اطلاعات
data = pd.read_csv("E:\\.vscode\\weatherPrediction\\datas3.csv")
...
model.fit(trainX, trainY, epochs=20, batch_size=32, validation_data=(testX, testY), callbacks=[early_stopping])
...
#نمایش روی نمودار
trainPredictPlot = np.empty((len(data['temperature_2m_mean']), 1))
trainPredictPlot[:,:]=np.nan
trainPredictPlot[look_back:len(trainpredict)+look_back,:]=trainpredict

testPredictPlot = np.empty((len(data['temperature_2m_mean']), 1))
testPredictPlot[:,:]=np.nan
testPredictPlot[len(trainpredict)+(look_back*2)+1:len(data['temperature_2m_mean'])-1,:]=testpredict

plt.plot(data["time"],data['temperature_2m_mean'],label='Data',linestyle='--')
plt.plot(data["time"],trainPredictPlot,label='Train Predictions')
plt.plot(data["time"],testPredictPlot,label='Test Predictions')
